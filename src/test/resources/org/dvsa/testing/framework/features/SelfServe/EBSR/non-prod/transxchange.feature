Feature: Test happy and unhappy paths in TransXchange functionality.
  ...
  @TransXchanged
  Scenario: Upload valid operator XML, create multiple pdf requests that use that same operator XML
    Given I generate an OAuth token
    When I upload valid "oneFileTimetable" operator xml into the bucket
    And I send a POST request to the API gateway with valid "oneFileTimetable" XML
    And I get the filename out of the success message in the output queue
    Then I verify the file is in the bucket
    And I send a POST request to the API gateway with valid "oneFileDvsaRecord" XML
    And I get the filename out of the success message in the output queue
    Then I verify the file is in the bucket
#    And I clean up the file generated by this test
